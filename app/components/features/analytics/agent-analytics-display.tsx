...
import { BookOpen, CheckCircle, TrendingUp, Users, Target, Brain } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground">of {summary.totalLessons}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summary.averageQuizCompletionRate}%</div>
            <div className="text-xs text-muted-foreground">{summary.totalCompletedQuizzes} of {summary.totalQuizzes} quizzes</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{summary.overallAverageQuizScore}%</div>
            <div className="text-xs text-muted-foreground">across all quizzes</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground">{summary.totalCorrectAnswers} of {summary.totalQuestionsAnswered} correct</div>
          </CardContent>
        </Card>
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                      <div>Lessons: {course.completedLessons}/{course.totalLessons}</div>
                      <div>Quizzes: {course.completedQuizzes}/{course.totalQuizzes}</div>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <div className="text-sm font-medium">Lessons: {course.progressPercentage}%</div>
                  ...
                  <div className="flex flex-col gap-1">
                    <Badge variant={course.progressPercentage === 100 ? "default" : "secondary"} className="text-xs">
                      {course.progressPercentage === 100 ? "Lessons Done" : "In Progress"}
                    </Badge>
                    {course.completedQuizzes > 0 && (
                      <Badge variant={course.averageQuizScore >= 80 ? "default" : "secondary"} className="text-xs">
                        {course.averageQuizScore >= 80 ? "Great Score" : "Improving"}
                      </Badge>
                    )}
                  </div>
                </div>
