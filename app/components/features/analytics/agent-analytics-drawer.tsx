...
export default function AgentAnalayticsDrawer({
	studentId,
}: { studentId: string }) {
							...
							>
								<div className="px-4 pb-2">
									<TabsList
										...
										style={{
											gridTemplateColumns: `repeat(${1 + fetcher.data.courseCompletionAnalytics.courses.length}, 1fr)`,
										}}
									>
										<TabsTrigger value="overview">Overview</TabsTrigger>
										{fetcher.data.courseCompletionAnalytics.courses.map(
											(course) => (
												<TabsTrigger
													key={course.id}
													value={course.id}
								...
								<div className="flex-1 overflow-hidden">
									<TabsContent
										value="overview"
										className="h-full overflow-y-auto px-4 mt-0"
									>
