...
export async function getAgentCourses(request: Request, studentId: string) {
	...
	try {
		...
		if (studentCourses.length === 0) {
			return data(
				{ success: true, data: [], message: "No courses found for this agent" },
				{ status: 200 },
			);
		}
		...
		return data(
			{
				success: true,
				data: courses,
				message: "Agent courses fetched successfully",
			},
			{ status: 200 },
		);
	} catch (error) {
		const errorMessage =
			error instanceof Error ? error.message : "Failed to get agent courses";
