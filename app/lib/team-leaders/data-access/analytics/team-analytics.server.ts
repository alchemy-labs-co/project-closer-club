...
export async function getTeamLeaderAnalytics(request: Request): Promise<TeamLeaderAnalytics> {
    ...
    try {
        ...
        const totalProgress = agentAnalytics.reduce((sum, agent) => sum + agent.summary.averageProgress, 0);
        const averageProgress = agentAnalytics.length > 0 ? Math.round(totalProgress / agentAnalytics.length) : 0;
