...
export async function submitQuiz(request: Request, formData: FormData) {
	...
	try {
		...
		const parsedSelectedAnswers = JSON.parse(
			selectedAnswers as string,
		) as number[];

		const { data: unvalidatedFields, success: isFormDataValid } =
		...
		if (!hasPassedQuiz) {
			...
			const incorrectQuestions = questions
				...
				FetcherSubmitQuizResponse["incorrectQuestions"]
			>;
			...
			return data(
				{
					success: true,
					message:
						"Quiz submitted. Some answers were incorrect. Please try again.",
					passed: false,
					score,
