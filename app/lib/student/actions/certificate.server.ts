...
const CERTIFICATE_TEMPLATE_PATH =
	"Blue%20and%20Gold%20Simple%20Certificate.png";
...
async function fetchCertificateImage(
	...
	if (!response.ok) {
		throw new Error(
			`Failed to fetch certificate image: ${response.status} - ${response.statusText}`,
		);
	}
...
export async function handleDownloadCertificate(
	...
	const { studentId, courseName, studentName, courseSlug } =
	...
	try {
		...
		const [{ id: courseId }] = await db
			...
			.from(coursesTable)
			.where(eq(coursesTable.slug, validatedFields.courseSlug))
			.limit(1);
		...
		if (
			existingCertificate.length > 0 &&
			existingCertificate[0].certificateUrl
		) {
			return {
				success: true,
		...
		const certificateImageBuffer =
			await fetchCertificateImage(certificateImageUrl);
