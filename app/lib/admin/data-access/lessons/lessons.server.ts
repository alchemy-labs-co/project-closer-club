...
export async function getAttachmentsForLesson(request: Request, lessonSlug: string, moduleSlug: string, courseSlug: string): Promise<{ success: boolean, attachments: Attachment[] | null }> {
    const { isLoggedIn } = await isAdminLoggedIn(request);
    if (!isLoggedIn) {
        throw redirect("/admin/login");
    }
    ...
    try {
        const { lesson } = await getLessonBySlug(request, lessonSlug, moduleSlug, courseSlug);
        if (!lesson) {
            throw redirect(`/dashboard/courses/${courseSlug}/${moduleSlug}`);
        }

        const attachments = await db.select().from(attachmentsTable).where(eq(attachmentsTable.lessonId, lesson.id));
...
export async function getQuizzesForLesson(request: Request, lessonSlug: string, moduleSlug: string, courseSlug: string): Promise<{ success: boolean, quizzes: Quiz[] | null }> {
    const { isLoggedIn } = await isAdminLoggedIn(request);
    if (!isLoggedIn) {
        throw redirect("/admin/login");
    }
