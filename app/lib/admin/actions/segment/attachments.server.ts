...
import { deleteAttachmentFromBunny, uploadAttachmentToBunny } from "~/lib/bunny.server";
import {
    ...
    try {
        ...
        for (const file of validAttachments) {
            ...
            if (!ACCEPTED_FILE_TYPES.includes(file.type)) {
                return data(
                    { success: false, message: `File "${file.name}" is not a supported type. Allowed types: PDF, DOCX, DOC, PNG, JPG, JPEG` },
                    { status: 400 }
                );
            }
        }
        ...
        const uploadPromises = validAttachments.map(async (file) => {
            const { cdnUrl, fileExtension, fileName } = await uploadAttachmentToBunny(file, lessonId);
        ...
        return data(
            { success: true, message: `Successfully uploaded ${validAttachments.length} attachment(s)` },
            { status: 200 }
        );
...
export async function deleteAttachment(request: Request, formData: FormData) {
    ...
    try {
        ...
        const [attachment] = await db.select().from(attachmentsTable).where(eq(attachmentsTable.id, attachmentId)).limit(1);
        if (!attachment) {
            return data(
        ...
        try {
            ...
            console.error("Failed to delete from Bunny storage, proceeding with database deletion:", bunnyError);
