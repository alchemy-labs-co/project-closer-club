...
function nonCriticalLoaderData(
	...
	const certificate = getStudentCertificateForCourse(
		request,
		studentId,
		courseSlug,
	);
...
function MarkCourseAsCompleted() {
	const { studentId, courseSlug, nonCriticalData, studentName } =
		useLessonLoaderData();
				...
				<div className="text-center p-6 border border-red-200 rounded-lg bg-red-50">
					<div className="space-y-2">
						...
						<p className="text-sm text-red-600">
							{fetcherData.message ||
								"An error occurred while generating your certificate."}
						</p>
					</div>
				</div>
				<fetcher.Form
					method="post"
					action="/resource/certificate"
					className="w-full"
				>
